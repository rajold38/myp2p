...
async function maybeSendButtons(firebaseUid, type, req, cbId) {
  ...
  const tx = await reqRef.transaction(cur => {
    if (!cur) return; // gone
    if (cur.botMsgId) return; // already sent
    if (cur._sending) return; // another worker is sending
  ...
  if (claimed.botMsgId) {
    sentByCbId.set(cbId, { firebaseUid, type, msgId: claimed.botMsgId, hid: claimed.hid, amt: parseFloat(claimed.amt), coin: claimed.coin });
    return;
  }
  ...
  if (msgId) {
    await reqRef.update({ botMsgId: msgId, cbId, _sending: null, _sendingTs: null });
