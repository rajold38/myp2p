...
async function handleApprove(fuid, type, req, cbId) {
  ...
  const user = (await db.ref(`users/${fuid}`).once('value')).val() || {};
  await sendResolutionEmail({ type, action: 'approve', user, uid: user.uid, amt, coin, oldBal: result?.oldBal, newBal: result?.newBal });
...
async function handleReject(fuid, type, req, cbId) {
  ...
  await db.ref(`users/${fuid}/pendingReqs/${type}/${cbId}`).remove();
  const user = (await db.ref(`users/${fuid}`).once('value')).val() || {};
  await sendResolutionEmail({ type, action: 'reject', user, uid: user.uid, amt, coin, oldBal: result?.oldBal, newBal: result?.newBal });
...
async function handleApproveRejectCb(cb, action, cbId) {
  ...
  await tgAnswer(cb.id, action === 'approve' ? '✅ Approved!' : '❌ Rejected!');
...
async function sendResolutionEmail(ctx) {
  ...
  if (!mailer) {
    log('MAIL', `skip ${label} UID=${uid} → ${to} — SMTP not configured`);
    return false;
  }
