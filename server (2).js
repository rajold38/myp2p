// ════════════════════════════════════════════════════════════════════
// PRO P2P Terminal — Telegram Backup Bot (24/7) — v3.1 (FIXED)
// FIXES IN v3.1:
//   1. Idempotent approve/reject — Firebase transaction prevents double balance
//   2. Callback-query dedup — Telegram retries no longer cause 2x messages
//   3. Persistent lastUpdateId in Firebase — restart doesn't replay old callbacks
//   4. Single-instance guard — only newest bot instance processes (kills duplicate
//      messages when Render runs old + new container during deploy)
//   5. Pending-request send dedup — botMsgId check is now atomic via transaction
// ════════════════════════════════════════════════════════════════════
...
function logIST(tag, msg) { console.log(`[${nowIST()} IST] [${tag}] ${msg}`); }
...
logIST('INIT', `✅ Firebase Admin initialized | instance=${INSTANCE_ID}`);

// ─── SINGLE-INSTANCE GUARD ──────────────────────────────────────────
// Stores the newest bot instance ID. If another instance starts (e.g. Render
// rolling deploy), the OLDER one will see a different value and stop polling.
async function claimInstanceLock() {
  await db.ref('botMeta/activeInstance').set({ id: INSTANCE_ID, ts: BOT_START_TIME });
  logIST('INIT', `🔒 Claimed instance lock`);
}
async function amIActive() {
  const snap = await db.ref('botMeta/activeInstance').once('value');
  const v = snap.val();
...
async function loadLastUpdateId() {
  const snap = await db.ref('botMeta/lastUpdateId').once('value');
  return parseInt(snap.val() || 0, 10);
}
...
// ─── CALLBACK DEDUP (kills TG retry duplicates) ─────────────────────
const processedCbIds = new Map(); // cb.id -> ts
...
function alreadyProcessed(id) { return processedCbIds.has(id); }

// ─── HELPERS ────────────────────────────────────────────────────────
const sentByCbId = new Map();
// In-process guard: cbIds for which THIS process has already pushed the
// button-message to Telegram. Prevents any chance of a same-process
// duplicate even if multiple listener events fire before botMsgId is written.
const locallySentCbIds = new Set();
const STALE_SENDING_MS = 30_000; // _sending lock TTL
...
// ─── PENDING REQUEST WATCHER (ATOMIC) ───────────────────────────────
async function maybeSendButtons(firebaseUid, type, req, cbId) {
  if (!req || !cbId) return;
  // Client (browser) writes the sent message id as `msgId`. Treat either
  // `botMsgId` (server-sent) or `msgId` (client-sent) as proof a button
  // message already exists — fixes the 2-3x duplicate-spam bug.
  const existingMsgId = req.botMsgId || req.msgId;
  if (existingMsgId) {
    sentByCbId.set(cbId, { firebaseUid, type, msgId: existingMsgId, hid: req.hid, amt: parseFloat(req.amt), coin: req.coin });
    locallySentCbIds.add(cbId);
    return;
  }
  if (locallySentCbIds.has(cbId)) return; // already sent in this process
  // ATOMIC CLAIM via transaction — only one listener (and only one instance) wins
  const reqRef = db.ref(`users/${firebaseUid}/pendingReqs/${type}/${cbId}`);
  const tx = await reqRef.transaction(cur => {
    if (!cur) return; // gone
    if (cur.botMsgId || cur.msgId) return; // already sent (server or client)
    // honor _sending lock unless it's stale (crashed worker)
  ...
  if (claimed.botMsgId || claimed.msgId) {
    sentByCbId.set(cbId, { firebaseUid, type, msgId: claimed.botMsgId || claimed.msgId, hid: claimed.hid, amt: parseFloat(claimed.amt), coin: claimed.coin });
    return;
  }
  // mark locally BEFORE the HTTP call so concurrent invocations in the
  // same process can't race past us
  locallySentCbIds.add(cbId);

  const userSnap = await db.ref(`users/${firebaseUid}`).once('value');
  const user = userSnap.val() || {};
  const text = fmtMsg(type, user, claimed);
  const msgId = await tgSendButtons(text, cbId);
  if (msgId) {
    await reqRef.update({ botMsgId: msgId, msgId: msgId, cbId, _sending: null, _sendingTs: null });
    sentByCbId.set(cbId, { firebaseUid, type, msgId, hid: claimed.hid, amt: parseFloat(claimed.amt), coin: claimed.coin });
    logIST(type.toUpperCase(), `UID=${user.uid} Name=${user.name||'—'} Amt=${claimed.amt} ${claimed.coin||'USDT'} cbId=${cbId} → TG msg sent (msgId: ${msgId})`);
  } else {
...
// Uses a transaction on the pendingReq node to "claim" it once.
// If the request is already claimed/gone, the function returns null and
// no balance change happens — fixes the double-balance bug.
async function claimPendingReq(firebaseUid, type, cbId, action) {
  const reqRef = db.ref(`users/${firebaseUid}/pendingReqs/${type}/${cbId}`);
  const tx = await reqRef.transaction(cur => {
    if (!cur) return; // gone — already processed
    if (cur._resolved) return; // already approved/rejected by another worker
  ...
  if (v._resolvedBy !== INSTANCE_ID) return null; // we did not win the race
  return v;
}

async function handleApprove(firebaseUid, type, ctx) {
  const claimed = await claimPendingReq(firebaseUid, type, ctx.cbId, 'approve');
  if (!claimed) { logIST('IDEMP', `approve skipped (already resolved) cbId=${ctx.cbId}`); return null; }
  ...
  let newBal = oldBal;
  if (type === 'dep') {
    // Atomic balance increment to prevent races with other balance writes
    const tx = await userRef.child('balance').transaction(cur => parseFloat(((parseFloat(cur || 0)) + ctx.amt).toFixed(8)));
    newBal = parseFloat(tx.snapshot.val() || 0);
    await userRef.child('balances/USDT').set(newBal);
  }
  await updateHistoryStatus(firebaseUid, ctx.hid, 'COMPLETED');
  await db.ref(`users/${firebaseUid}/pendingReqs/${type}/${ctx.cbId}`).remove();
  logIST('APPROVE', `UID=${user.uid} ${type.toUpperCase()} ${ctx.amt} USDT | OldBal=${oldBal.toFixed(2)} → NewBal=${newBal.toFixed(2)}`);
  return { oldBal, newBal, user };
}

async function handleReject(firebaseUid, type, ctx) {
  const claimed = await claimPendingReq(firebaseUid, type, ctx.cbId, 'reject');
  if (!claimed) { logIST('IDEMP', `reject skipped (already resolved) cbId=${ctx.cbId}`); return null; }
  ...
  await db.ref(`users/${firebaseUid}/pendingReqs/${type}/${ctx.cbId}`).remove();
  logIST('REJECT', `UID=${user.uid} ${type.toUpperCase()} ${ctx.amt} USDT ${type==='wit'?'refunded':''} | OldBal=${oldBal.toFixed(2)} → NewBal=${newBal.toFixed(2)}`);
  return { oldBal, newBal, user };
}
...
async function pollUpdates() {
  if (!(await amIActive())) {
    logIST('POLL', '⏸️ another instance is active — sleeping');
...
async function handleCallback(cb) {
  // Dedup: Telegram retries deliver the same cb.id again
  if (alreadyProcessed(cb.id)) {
    logIST('DEDUP', `cb ${cb.id} already processed — skipping`);
...
async function handleApproveRejectCb(cb, action, cbId) {
  ...
  if (action === 'approve') {
    ...
    if (!r) { await tgAnswer(cb.id, '⚠️ Already resolved'); return; }
...
async function closeAllP2PForUser(uid) {
  ...
  for (const [oid, ord] of Object.entries(orders)) {
    ...
    if (ord.mode === 'BUY' && ord.usdt) {
      const userRef = db.ref(`users/${found.firebaseUid}`);
      const tx = await userRef.child('balance').transaction(cur => parseFloat(((parseFloat(cur||0)) + parseFloat(ord.usdt)).toFixed(8)));
      const newBal = parseFloat(tx.snapshot.val() || 0);
      await userRef.child('balances/USDT').set(newBal);
    ...
    for (const type of ['dep', 'wit']) {
      ...
      for (const [cbId, req] of Object.entries(map)) {
        ...
        if (cbId === targetId || req.cbId === targetId || req.hid === targetId) {
          ...
          if (type === 'wit' && req.amt) {
            const userRef = db.ref(`users/${fuid}`);
            const tx = await userRef.child('balance').transaction(cur => parseFloat(((parseFloat(cur||0)) + parseFloat(req.amt)).toFixed(8)));
            const newBal = parseFloat(tx.snapshot.val() || 0);
            await userRef.child('balances/USDT').set(newBal);
          }
...
async function handleCloseOrder(orderId) {
  ...
  let done = false;
  const users = [];
  snap.forEach(child => users.push({ key: child.key, val: child.val() || {} }));
    ...
    for (const [oid, ord] of Object.entries(orders)) {
      if (oid === orderId || String(ord?.id) === orderId) {
        ...
        if (refund > 0) {
          ...
          const tx = await userRef.child('balance').transaction(cur => parseFloat(((parseFloat(cur||0)) + refund).toFixed(8)));
          const newBal = parseFloat(tx.snapshot.val() || 0);
          await userRef.child('balances/USDT').set(newBal);
        }
...
async function handleUpdate(upd) {
  ...
  let m;
  if ((m = text.match(/^\/user\s+([A-Z0-9]{2,15})$/i))) return sendUserDetailCard({ id: 'cmd', message: msg }, m[1].toUpperCase());
  if ((m = text.match(/^\/history\s+([A-Z0-9]{2,15})$/i))) return sendUserHistory(m[1].toUpperCase(), 15);
  if ((m = text.match(/^\/credit\s+([A-Z0-9]{2,15})\s+([\d.]+)$/i))) return adminCreditDebit(m[1].toUpperCase(), parseFloat(m[2]), '+', 'ADMIN_CREDIT');
  if ((m = text.match(/^\/debit\s+([A-Z0-9]{2,15})\s+([\d.]+)$/i))) return adminCreditDebit(m[1].toUpperCase(), parseFloat(m[2]), '-', 'ADMIN_DEBIT');
  if ((m = text.match(/^\/balance\s+([A-Z0-9]{2,15})\s+([\d.]+)$/i))) return adminSetExactBalance(m[1].toUpperCase(), parseFloat(m[2]));
  if ((m = text.match(/^\/setbalance\s+([A-Z0-9]{2,15})\s+([A-Z]{2,8})\s+([\d.]+)$/i))) return handleSetBalancePerCoin(m[1].toUpperCase(), m[2].toUpperCase(), parseFloat(m[3]));
  if ((m = text.match(/^\/ban\s+([A-Z0-9]{2,15})$/i))) return adminBan(m[1].toUpperCase(), true);
  if ((m = text.match(/^\/unban\s+([A-Z0-9]{2,15})$/i))) return adminBan(m[1].toUpperCase(), false);
  ...
  if ((m = text.match(/^#([A-Z0-9]{2,15})\s+([\d.]+)$/i))) return quickCredit(m[1].toUpperCase(), parseFloat(m[2]));

  // ── ADMIN P2P CHAT REPLIES ──────────────────────────────────────
  // If the admin replies to a deal-bridge message (or types text mentioning
  // an order id / UID), persist the message into the user's chat state in
  // Firebase so the user sees it next time they open the deal — even if
  // their app was closed when the reply was sent.
  if (text && !text.startsWith('/')) {
    await persistAdminChatReply(msg).catch(e => logIST('CHAT', `persist err: ${e.message}`));
  }
}

// Find the user + order whose chat state matches this incoming admin
// message (via reply_to_message_id, order id, or UID mention) and append
// it atomically to that order's chat messages.
async function persistAdminChatReply(msg) {
  const text = (msg.text || '').trim();
  if (!text) return;
  const replyToId = msg.reply_to_message?.message_id || null;
  const tgId = msg.message_id;

  const snap = await db.ref('users').once('value');
  if (!snap.exists()) return;
  let target = null; // { fuid, orderId, merchant }
  snap.forEach(child => {
    if (target) return;
    const u = child.val() || {};
    const chats = u.chats || {};
    const orders = u.orders || {};
    for (const [orderId, st] of Object.entries(chats)) {
      if (!st || typeof st !== 'object') continue;
      let match = false;
      if (replyToId && Number(st.tgMsgId) === Number(replyToId)) match = true;
      if (!match && text.includes(String(orderId))) match = true;
      if (!match && u.uid && text.toUpperCase().includes(String(u.uid).toUpperCase())) match = true;
      if (match) {
        const ord = orders[orderId];
        target = { fuid: child.key, orderId, merchant: ord?.merchant || 'Merchant' };
        break;
      }
    }
  });
  ...
  // Strip the order-id / UID prefix the admin may have typed
  let clean = text.replace(new RegExp(`^#?${target.orderId}[\\s:,\\-]*`, 'i'), '').trim();
  const userSnap = await db.ref(`users/${target.fuid}/uid`).once('value');
  const uidVal = userSnap.val();
  if (uidVal) clean = clean.replace(new RegExp(`^${uidVal}[\\s:,\\-]*`, 'i'), '').trim();
  if (!clean) return;

  const stateRef = db.ref(`users/${target.fuid}/chats/${target.orderId}`);
  const tx = await stateRef.transaction(cur => {
    const s = cur || { messages: [], tgMsgId: null, cbId: null, dealStage: 'init', upiSubmitted: false, seenMsgIds: [] };
    s.messages = Array.isArray(s.messages) ? s.messages : [];
    s.seenMsgIds = Array.isArray(s.seenMsgIds) ? s.seenMsgIds : [];
    if (s.seenMsgIds.includes(tgId)) return; // already saved by a client
    s.messages.push({ type: 'them', sender: target.merchant, text: clean, ts: Date.now() });
    s.seenMsgIds.push(tgId);
    return s;
  });
  if (tx.committed) logIST('CHAT', `admin reply → ${target.fuid}/${target.orderId}: "${clean.slice(0,40)}"`);
}
...
async function adminCreditDebit(uid, amt, sign, type) {
  ...
  const tx = await userRef.child('balance').transaction(cur => {
    ...
    return sign === '+' ? parseFloat((c + amt).toFixed(8)) : Math.max(0, parseFloat((c - amt).toFixed(8)));
  });
  const newBal = parseFloat(tx.snapshot.val() || 0);
  await userRef.child('balances/USDT').set(newBal);
...
async function quickCredit(uid, amt) {
  ...
  const userRef = db.ref(`users/${found.firebaseUid}`);
  const oldBal = parseFloat(found.user.balance || 0);
  const tx = await userRef.child('balance').transaction(cur => parseFloat(((parseFloat(cur||0)) + amt).toFixed(8)));
  const newBal = parseFloat(tx.snapshot.val() || 0);
  await userRef.child('balances/USDT').set(newBal);
  await db.ref(`users/${found.firebaseUid}/history`).push({
    hid: 'h_admin_' + Date.now() + '_' + Math.random().toString(36).slice(2, 5),
...
(async () => {
  ...
  logIST('INIT', `📍 Resumed from lastUpdateId=${lastUpdateId}`);
...
// ─── EXPRESS HEALTH ─────────────────────────────────────────────────
const app = express();
app.get('/', (req, res) => res.send('PRO P2P Bot v3.1 running ✅'));
app.get('/health', (req, res) => res.json({
  ok: true, uptime: process.uptime(), lastUpdateId, cached: sentByCbId.size,
  instance: INSTANCE_ID, started: new Date(BOT_START_TIME).toISOString()
}));
