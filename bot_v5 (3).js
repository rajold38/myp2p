  ...
  for (const [firebaseUid, userData] of Object.entries(users)) {
    ...
    if (orders && typeof orders === "object") {
      for (const [ordId, order] of Object.entries(orders)) {
        ...
        // New source of truth = actionRequestId from frontend. This avoids missed alerts
        // when chat state gets updated multiple times around the same user action.
        const cs = chats?.[ordId];
        if (cs && cs.dealStage === "pending_verify") {
          const requestId = cs.actionRequestId || null;
          const needsButtons = requestId
            ? cs.lastNotifiedActionRequestId !== requestId
            : !cs.cbId;
          ...
          if (needsButtons) {
            ...
            const msgId = await tgButtons(text, cbId);
            if (msgId) {
              await fbPatch(`users/${firebaseUid}/chats/${ordId}`, { cbId, tgMsgId: msgId, lastNotifiedActionRequestId: requestId || cbId });
              console.log(`🤝 Trade ${isBuy ? "PAID-claim" : "RELEASE-request"} #${order.id} → ${userData.uid} cbId=${cbId} req=${requestId || "legacy"}`);
            } else {
              console.log(`⚠️ Trade button send failed for #${order.id} req=${requestId || "legacy"}`);
            }
          }
        }
      }
