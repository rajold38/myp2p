...
// Each chain lists 2+ candidate RPCs. publicnode.com is used first — it's
// built for programmatic/server access (unlike llamarpc.com, which puts
// server-side requests behind a Cloudflare "Just a moment..." bot-check and
// returns 403). If the first URL is ever down, the engine automatically
// falls back to the next one and logs which one it switched to.
// Override with a comma-separated list via env var, e.g.
//   ETH_RPC_URL=https://myprovider.com/key,https://ethereum-rpc.publicnode.com
const parseRpcList = (envVal, fallbackList) =>
  (envVal ? envVal.split(',').map((s) => s.trim()).filter(Boolean) : fallbackList);

const EVM_CHAINS = {
  1:     { name: 'ETH',       coin: 'ETH',
           rpcs: parseRpcList(process.env.ETH_RPC_URL, ['https://ethereum-rpc.publicnode.com', 'https://rpc.ankr.com/eth', 'https://cloudflare-eth.com']) },
  56:    { name: 'BSC',       coin: 'BNB',
           rpcs: parseRpcList(process.env.BSC_RPC_URL, ['https://bsc-rpc.publicnode.com', 'https://bsc-dataseed.binance.org/', 'https://rpc.ankr.com/bsc']) },
  137:   { name: 'POLYGON',   coin: 'POL',
           rpcs: parseRpcList(process.env.POLYGON_RPC_URL, ['https://polygon-bor-rpc.publicnode.com', 'https://polygon-rpc.com', 'https://rpc.ankr.com/polygon']) },
  43114: { name: 'AVALANCHE', coin: 'AVAX',
           rpcs: parseRpcList(process.env.AVALANCHE_RPC_URL, ['https://avalanche-c-chain-rpc.publicnode.com', 'https://api.avax.network/ext/bc/C/rpc']) },
};
...
export function mountDepositEngine(app, ctx) {
  ...
  // ── EVM providers, one per candidate RPC per chain, created once and reused ──
  const providers = {}; // chainId -> [provider, provider, ...] in priority order
  for (const [chainId, meta] of Object.entries(EVM_CHAINS)) {
    providers[chainId] = meta.rpcs.map((url) => {
      try {
        return new ethers.JsonRpcProvider(url, Number(chainId), { staticNetwork: true });
      } catch (error) {
        warn('WALLET', `could not create RPC provider for ${meta.name} (${url}): ${error.message}`);
        return null;
      }
    }).filter(Boolean);
  }

  // Tries each configured RPC for a chain in order, returns the first one that
  // actually responds. Logs a warning naming exactly which URL(s) failed.
  async function getHealthyProvider(chainId, meta) {
    const list = providers[chainId] || [];
    for (let i = 0; i < list.length; i++) {
      try {
        await list[i].getBlockNumber();
        if (i > 0) warn('POLL', `${meta.name}: primary RPC unreachable, using fallback "${meta.rpcs[i]}" instead`);
        return list[i];
      } catch (error) {
        warn('POLL', `${meta.name}: RPC "${meta.rpcs[i]}" unreachable — ${String(error.message || error).slice(0, 180)}`);
      }
    }
    return null;
  }
  ...
  async function pollEvmChain(chainId, meta, addressSet, addressToUid) {
    ...
    const provider = await getHealthyProvider(chainId, meta);
    if (!provider) { warn('POLL', `${meta.name}: ALL configured RPCs unreachable this cycle (tried ${meta.rpcs.join(', ')}), skipped`); return { ok: false, newDeposits: 0 }; }
    ...
    try {
      ...
      warn('POLL', `${meta.name}: RPC became unreachable mid-cycle — ${error.message}`);
  ...
  app.get('/api/debug/status', (req, res) => {
    ...
    res.json({
      ...
      chainsConfigured: Object.fromEntries(Object.entries(EVM_CHAINS).map(([id, m]) => [id, { name: m.name, rpcsConfigured: m.rpcs, providersReady: (providers[id] || []).length }])),
